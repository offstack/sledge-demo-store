...
class QuantityInput extends HTMLElement {
  ...
  onButtonClick(event) {
    ...
    if (document.getElementById("Quantity-main-product")) {
      const quantityInput = Number(
        document.getElementById("Quantity-main-product").value
      );

      const checkoutButton = document.getElementById("checkout-button");

      const quantity =
        event.target.name === "plus" ? quantityInput + 1 : quantityInput - 1;

      checkoutButton.action =
        checkoutButton.action.split(":")[1] + ":" + quantity;
    }
...
class VariantSelects extends HTMLElement {
  ...
  updateStockInfo() {
    if (!this.currentVariant) return;
    document.querySelectorAll(".selected-inventory-quantity").forEach((el) => {
      el.innerText = this.currentVariant.inventory_quantity;
    });

    mainProductQuantityInput.attributes["data-max-quantity"].value =
      this.currentVariant.inventory_quantity;
    mainProductQuantityInput.value = 1;
  }

  checkoutButton() {
    const quantity = document.getElementById("Quantity-main-product").value;
    document.getElementById(
      "checkout-button"
    ).href = `/cart/${this.currentVariant.id}:${quantity}`;
  }
  ...
  getVariantData() {
    let getStock = JSON.parse(
      document.querySelector("#selected-variant-json").innerText
    );
    this.variantData =
      this.variantData ||
      JSON.parse(
        ...
        variant.inventory_quantity = getStock[index].inventory_quantity;
        return variant;
      });
...
const sortFunctions = () => {
  ...
  sortButton.addEventListener("click", function (el) {
  ...
  document.onclick = function (e) {
    if (e.target.id !== "sort-button") sortMenu.classList.add("hidden");
  };

  function renderSort(key) {
    ...
    if (history.pushState && history.replaceState) {
      var currentSort = document.location.href.split("?sort=")[1];
      var sectionUrl =
        currentSort === "manual"
          ? `/?sections=new-product`
          : `/?sections=new-product-sort-${currentSort}`;
      fetch(sectionUrl)
        .then((res) => {
          return res.json();
        })
        .then((response) => {
          var section = Object.keys(response)[0];
          document.getElementById("shopify-section-new-product").innerHTML =
            response[section];
          sortFunctions();
        });
    }
  ...
  sortMenu.querySelectorAll("a").forEach((el) => {
    el.addEventListener("click", function (e) {
      e.preventDefault();
      var sortVal = this.href.split("?sort=")[1];
      renderSort(sortVal);
    });
  });
};
